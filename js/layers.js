	...
	buyables: {
            0: {
                ...
                cost(x=player[this.layer].buyables[this.id]) { // cost for buying xth buyable, can be an object if there are multiple currencies
                    let cost = [new Decimal("1e800"),new Decimal("1e4000"),new Decimal("1e20000"),new Decimal("1e100000"),new Decimal("1e500000"),new Decimal(Infinity)][player[this.layer].buyables[this.id].toNumber()];
                    return cost
                },
            ...
            5: {
                ...
                cost(x=player[this.layer].buyables[this.id]) { // cost for buying xth buyable, can be an object if there are multiple currencies
					...
					if(player[this.layer].buyables[this.id].gt(10.5))return new Decimal(Infinity);
            ...
            6: {
                title: "Upgrade", // Optional, displayed at the top in a larger font
                cost(x=player[this.layer].buyables[this.id]) { // cost for buying xth buyable, can be an object if there are multiple currencies
                    let cost = [new Decimal(0),new Decimal("1e525000"),new Decimal("1e650000"),new Decimal("1e875000"),new Decimal("e12e5"),/*new Decimal("e1625e3"),*/new Decimal(Infinity)][player[this.layer].buyables[this.id].toNumber()];
                    return cost
                },
                display() { // Everything else displayed in the buyable button after the title
                    let data = tmp[this.layer].buyables[this.id]
...
addLayer("tptc_l", {
	...
	buyables: {
            ...
            17: {
				...
				effect(){
					let x=player[this.layer].buyables[this.id].mul(player.tptc_l.power.add(1).log10().add(1));
					return Decimal.pow(1.25,x.pow(0.4));
				},
...
addLayer("tptc_i", {
	...
	buyables: {
            ...
            11: {
                ...
                cost(x=player[this.layer].buyables[this.id]) { // cost for buying xth buyable, can be an object if there are multiple currencies
                    let cost = Decimal.pow(2, x.pow(0.54)).ceil();
                    return cost
                },
                display() { // Everything else displayed in the buyable button after the title
                    let data = tmp[this.layer].buyables[this.id]
...
addLayer("tptc_mb", {
	...
	if(l=="tptc_p" || l=="tptc_b" || l=="tptc_g" || l=="tptc_t" || l=="tptc_e" || l=="tptc_s" || l=="tptc_sb" || l=="tptc_sg" || l=="tptc_h" || l=="tptc_q" || l=="tptc_hb" || l=="tptc_ss" || l=="tptc_m" || l=="tptc_ba" || l=="tptc_ps"  || l=="tptc_sp" || l=="tptc_l" || l=="tptc_hs" || l=="tptc_i" || l=="tptc_mb" || l=="tptc_ge" || l=="tptc_ma" || !l.startsWith("tptc_")){return;}
	 ...
	 tabFormat: ["main-display",
                    ...
                    ["blank", "5px"],
						"milestones",
					["display-text",function(){return "Mastery bricks remaining: "+formatWhole(player.tptc_mb.points.sub(tmp.tptc_mb.usedMB))+"/"+formatWhole(player.tptc_mb.points)}],
					["buyable",2],
						"buyables","upgrades"
				],
	milestones: {
	...
	buyables: {
            ...
            12: {
				...
				effect() {
					let ret=player.tptc_mb.buyables[12].sub(3).max(0).add(1);
					return ret;
                },
				nextat() {
...
addLayer("forest_c", {
    ...
    getNextAt() {
		...
		return new Decimal(11).add(player.forest_c.points.times(Decimal.max(10,player.forest_c.points.mul(scaling)))).ceil();
    },
    gainMult() { // Calculate the multiplier for main currency from bonuses
        mult = new Decimal(1)
		return mult
    ...
    effectDescription() {
	 ...
	 upgrades: {
			...
			21: {
				...
				effect() {
					let base=1.2;
					if(hasUpgrade("burning_c",22))base=1.5;
                    let ret = Decimal.pow(base,Decimal.log10(player.burning_c.best.add(3)).pow(0.9));
                    //if (ret.gte("1e20000000")) ret = ret.sqrt().times("1e10000000")
                    return ret;
				},
...
addLayer("burning_e", {
	...
	allocationPower(){
		if(hasUpgrade("burning_e",21))return 1.3;
		return 1;
	},
	...
	tooltip(){
	 ...
	 upgrades: {
			...
			32: {
                ...
                description(){
					if(hasUpgrade("incrementy_a",25))return "Each Incrementy Speed adds .01 to the Incrementy Speed base";
					return "Each Incrementy Speed adds .01 to the Incrementy Speed base (capped at 10)";
				},
	 ...
	 buyables:{
		 ...
		 11: {
                ...
                display() { // Everything else displayed in the buyable button after the title
                    let data = tmp[this.layer].buyables[this.id]
                    return "Amount: "+formatWhole(player.incrementy_i.buyables[11])+"<br>"+
...
addLayer("incrementy_p", {
	...
	getResetGainReal() {
		...
		if(hasUpgrade("incrementy_n",15))ret=ret.log10().pow(2);
		else ret=ret.log10().div(20000).sqrt();
	...
	getNextAt() {
		...
		if(hasUpgrade("incrementy_n",15))ret=ret.sqrt();
		else ret=ret.pow(2).mul(20000);
		ret=Decimal.pow(10,ret);
		return ret;
	},
    ...
    layerShown(){return player.tm.currentTree==5 && player.tm.buyables[5].gte(10);},
	 ...
	 tabFormat: ["main-display",
                    ...
                    ["blank", "5px"],
						"milestones",
					["display-text",function(){return "Particle Cap: "+format(tmp.incrementy_p.getResetGainReal.mul(60))+" (based on your incrementy)"}],
					"upgrades"
				],
	 resetsNothing: true,
	passiveGeneration(){
	 ...
	 buyables:{
		 ...
		 11: {
				...
				effect(){
					...
					return Decimal.pow(base,player[this.layer].buyables[this.id].add(tmp[this.layer].buyables[this.id].free));
				},
				free(){
					let ret=player.incrementy_n.buyables[12];
					ret=ret.add(tmp.incrementy_n.buyables[12].free);
					return ret;
				}
        },
		 12: {
		 ...
		 13: {
                ...
                display() { // Everything else displayed in the buyable button after the title
                    let data = tmp[this.layer].buyables[this.id]
                    return "Amount: "+formatWhole(player.incrementy_n.buyables[13])+"<br>"+
					"Cost: "+format(data.cost)+" Neutrinos<br>"+
    ...
    hotkeys: [
	 ...
	 upgrades: {
			...
			11: {
				...
				realEffect() {
					...
					ret = ret.mul(buyableEffect("gd_r",11));
	 ...
	 update(diff){
		if(hasUpgrade("gd_u",11)){
			...
			if(player.gd_r.buyables[11].lt(1))player.modpoints[6]=f2(f1(player.modpoints[6],layers.gd_u.scstart()).add(tmp.gd_u.upgrades[11].realEffect.mul(diff)),layers.gd_u.scstart());
			else player.gd_r.refactored=f2(f1(player.gd_r.refactored,layers.gd_u.scstart()).add(tmp.gd_u.upgrades[11].realEffect.mul(diff)),layers.gd_u.scstart());
		}
	},
	scstart(){
		let ret=new Decimal(1);
...
addLayer("gd_r", {
	...
	buyables: {
            ...
            11: {
                ...
                display() { // Everything else displayed in the buyable button after the title
                    let data = tmp[this.layer].buyables[this.id]
                    return "Refactoring is "+(player[this.layer].buyables[11].lt(1)?"disabled":"enabled")+".\n\
				...
				effect(){
					return player[this.layer].refactored.add(10).log10().pow(player.gd_r.points.mul(0.1).add(1));
				},
                buyMax() {}, // You'll have to handle this yourself if you want
                style: {'height':'222px'},
            },
...
addLayer("gd_f", {
    ...
    layerShown(){return player.tm.currentTree==6 && player.tm.buyables[6].gte(4);},
	 ...
	 tabFormat: ["main-display",
		...
		fansEffect(){
			return player.gd_f.fans.add(9).log10();
		},
	buyables: {
            rows: 1,
            cols: 4,
            11: {
				...
				effect(){
					return player.gd_f.fans.add(9).log10().pow(player[this.layer].buyables[this.id].sqrt().mul(0.1));
				},
                buyMax() {}, // You'll have to handle this yourself if you want
                style: {'height':'222px'},
            },
            12: {
				...
				effect(){
					return player.gd_f.fans.add(9).log10().pow(player[this.layer].buyables[this.id].sqrt().mul(0.4));
				},
                buyMax() {}, // You'll have to handle this yourself if you want
                style: {'height':'222px'},
            },
            13: {
				...
				effect(){
					return player.gd_f.fans.add(9).log10().pow(player[this.layer].buyables[this.id].sqrt().mul(0.25));
				},
                buyMax() {}, // You'll have to handle this yourself if you want
                style: {'height':'222px'},
            },
            14: {
				...
				effect(){
					return player.gd_f.fans.add(9).log10().pow(player[this.layer].buyables[this.id].sqrt().mul(0.3));
				},
                buyMax() {}, // You'll have to handle this yourself if you want
                style: {'height':'222px'},
            },
	...
	milestones: {
		...
		canBuyMax() {return player.gd_f.best.gte(4)},
	 autoPrestige(){
		 return false;
	 },resetsNothing(){
		 return player.gd_f.best.gte(4);
	 },
});
	...
	buyables: {
            ...
            11: {
                ...
                display() { // Everything else displayed in the buyable button after the title
					...
					Classes: "+formatWhole(player[this.layer].buyables[this.id])+"\n\
				...
				effect(){
					return player.gd_s.buyables[11].mul(player.gd_s.points.mul(0.1).add(1)).pow(0.5).mul(0.1).add(1);
				},
                buyMax() {}, // You'll have to handle this yourself if you want
                style: {'height':'222px'},
            },
